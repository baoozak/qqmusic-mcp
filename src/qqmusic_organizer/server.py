...
def build_mcp(organizer: Organizer) -> FastMCP:
    ...
    local_change = ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=True)
    remote_add = ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=True)
    remote_delete = ToolAnnotations(readOnlyHint=False, destructiveHint=True, idempotentHint=False, openWorldHint=True)
    ...
    @mcp.tool(annotations=local_change)
    ...
    @mcp.tool(annotations=remote_add)
    ...
    @mcp.tool(annotations=remote_delete)
    ...
    @mcp.tool(annotations=local_change)
    async def qqmusic_prepare_incremental_organization(baseline_export_id: str | None = None) -> dict[str, Any]:
        """Snapshot liked songs and create a plan-ready export containing only additions since the baseline."""
        return await organizer.prepare_incremental_organization(baseline_export_id)

    @mcp.tool(annotations=local_change)
    async def qqmusic_enrich_export_page(
        export_id: str,
        page: int = 1,
        page_size: int = 20,
        ...
        """Batch-enrich one export page with cached QQ Music metadata and an optional lyrics excerpt."""
        return await organizer.enrich_export_page(export_id, page, page_size, include_lyrics, force_refresh)

    @mcp.tool(annotations=local_change)
    async def qqmusic_preview_smart_playlist_sync(
        name: str,
        source_directory_id: int,
        rule: SmartPlaylistRule,
        remove_extraneous: bool = False,
    ) -> dict[str, Any]:
        """Preview additions and optional removals for a persistent rule-based playlist without writing QQ Music."""
        return await organizer.preview_smart_playlist_sync(name, source_directory_id, rule, remove_extraneous)

    @mcp.tool(annotations=remote_delete)
    async def qqmusic_apply_smart_playlist_sync(smart_playlist_id: str, preview_sha256: str) -> dict[str, Any]:
        """Apply an unchanged smart-playlist preview; rerun preview if either playlist changed."""
        return await organizer.apply_smart_playlist_sync(smart_playlist_id, preview_sha256)
